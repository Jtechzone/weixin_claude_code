 ...
 */
export async function downloadAndDecryptBuffer(encryptedQueryParam, aesKeyBase64, cdnBaseUrl, label) {
 ...
 */
export async function downloadPlainCdnBuffer(encryptedQueryParam, cdnBaseUrl, label) {
    const url = buildCdnDownloadUrl(encryptedQueryParam, cdnBaseUrl);
    logger.debug(`${label}: fetching url=${url}`);
